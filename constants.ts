      ...
      steps: [
        {
          ...
          code: `import { AISuite } from './services/AISuite';\n\nconst agent = new AISuite(process.env.API_KEY);\n\n// Defining the ReAct Framework via System Instructions\nconst SYSTEM_PROMPT = \`You are a high-reasoning Agent.\nFor every request, follow this logical loop:\n1. Thought: Analyze the user's intent and plan steps.\n2. Action: Identify if a tool is needed (Action: [ToolName]).\n3. Observation: Process tool results.\n4. Conclusion: Provide the final answer.\n\nAlways use first-principles thinking.\`;\n\nasync function solveComplexTask(query: string) {\n  // 1. Initialize request with Native Thinking Budget (CoT)\n  const response = await agent.generate({\n    prompt: query,\n    systemInstruction: SYSTEM_PROMPT,\n    thinkingBudget: 2000 // Reserved tokens for internal reasoning\n  });\n\n  // 2. Extract and display the reasoning flow\n  const output = response.text;\n  \n  if (output?.includes('Thought:')) {\n    console.log("Agent is reasoning...");\n  }\n  \n  console.log("Final Agent Output:", output);\n  return output;\n}\n\n// Executable call\nsolveComplexTask("Is the square root of 144 a prime number? Show your reasoning.");`
        },
        {
        ...
        },
        {
          ...
          code: `import { AISuite } from './services/AISuite';\nimport { Type, FunctionDeclaration } from '@google/genai';\n\nconst agent = new AISuite(process.env.API_KEY);\n\n// 1. Quality API Definition\nconst databaseTool: FunctionDeclaration = {\n  name: "query_user_db",\n  description: "Queries internal database for user history. Required for personalized info.",\n  parameters: {\n    type: Type.OBJECT,\n    properties: {\n      user_id: { type: Type.STRING },\n      query_type: { type: Type.STRING, enum: ["purchases", "profile"] }\n    },\n    required: ["user_id", "query_type"]\n  }\n};\n\n// 2. The Execution Cycle\nasync function runAgent() {\n  const response = await agent.generate({\n    prompt: "Get purchase history for user_99.",\n    tools: [databaseTool]\n  });\n\n  if (response.functionCalls) {\n    const call = response.functionCalls[0];\n    console.log(\`Model requested: \${call.name} with \`, call.args);\n    // Execute local logic here...\n  }\n}\n\nrunAgent();`
        }
      ]
    },
    zh: {
      ...
      steps: [
        {
          ...
          description: "掌握思维链 (CoT) 和 ReAct 范式，通过系统提示词强制模型进行逻辑推理与自我迭代。",
          code: `import { AISuite } from './services/AISuite';\n\nconst agent = new AISuite(process.env.API_KEY);\n\n// 通过系统指令定义 ReAct 框架\nconst SYSTEM_PROMPT = \`你是一个具备深度推理能力的智能体。\n请遵循以下逻辑循环处理任务：\n1. 思考 (Thought): 分析意图并制定计划。\n2. 行动 (Action): 识别是否需要调用工具 (Action: [工具名])。\n3. 观察 (Observation): 处理工具返回的结果。\n4. 结论 (Conclusion): 给出最终答案。\n\n请始终使用第一性原理进行思考。\`;\n\nasync function solveComplexTask(query: string) {\n  // 1. 初始化请求并开启原生思维链 (Thinking Budget)\n  const response = await agent.generate({\n    prompt: query,\n    systemInstruction: SYSTEM_PROMPT,\n    thinkingBudget: 2000 // 为内部推理预留的 Token 预算\n  });\n\n  // 2. 提取并展示推理流\n  const output = response.text;\n  \n  if (output?.includes('思考:')) {\n    console.log("智能体正在进行深度推理...");\n  }\n  \n  console.log("智能体最终输出:", output);\n  return output;\n}\n\n// 执行示例\nsolveComplexTask("144 的平方根是质数吗？请展示你的推理过程。");`
        },
        {
          title: "输出格式强制约束 (JSON)",
        ...
        },
        {
          ...
          code: `import { AISuite } from './services/AISuite';\nimport { Type, FunctionDeclaration } from '@google/genai';\n\nconst agent = new AISuite(process.env.API_KEY);\n\n// 1. 高质量 API 定义规范\nconst databaseTool: FunctionDeclaration = {\n  name: "query_user_db",\n  description: "查询内部用户数据库以获取历史记录。个性化服务必选。",\n  parameters: {\n    type: Type.OBJECT,\n    properties: {\n      user_id: { type: Type.STRING },\n      query_type: { type: Type.STRING, enum: ["购买记录", "个人资料"] }\n    },\n    required: ["user_id", "query_type"]\n  }\n};\n\n// 2. 执行闭环逻辑\nasync function runAgent() {\n  const response = await agent.generate({\n    prompt: "获取用户 user_99 的购买历史。",\n    tools: [databaseTool]\n  });\n\n  if (response.functionCalls) {\n    const call = response.functionCalls[0];\n    console.log(\`模型请求执行: \${call.name}，参数: \`, call.args);\n    // 此处可编写本地函数执行代码并传回模型...\n  }\n}\n\nrunAgent();`
        }
      ]
    }
  },
